...
IPAddress ip(169,254,243,187);   // IP address, may need to change depending on network
...
AF_Stepper stepperX(48,2); // Define some steppers and the pins the will use
AF_Stepper stepperY(48,1);// Define some steppers and the pins the will use
Servo penServo;  //dotServo 라는 서보 모터 등록
...
void setup()
{
    ...
    Serial1.begin(115200); //setup keys sensor as inputs??
...
void loop()
{
  ...
  case 'a' : {
    PenPressing();
    stepperY.step(8,FORWARD,STEP);
    stepperY.step(8,FORWARD,STEP);   
    stepperX.step(8,FORWARD,STEP);
    stepperY.step(8,BACKWARD,STEP);
    stepperY.step(8,BACKWARD,STEP);
   
    stepperX.step(10,FORWARD,STEP);
    horsteps+=18;   
    CELL += 2;
    LINE+=2;
    break;
  ...
  if((CELL == 18)||(CELL == 19)){                 // 줄 바꿈
    stepperX.step(horsteps+18, BACKWARD, STEP);
    stepperY.step(32, FORWARD, STEP);
    CELL=0;
    horsteps=0;
  }
  if((LINE == 36) || (LINE == 35)){             // 쪽? 바꿈
    stepperX.step(horsteps, BACKWARD, STEP);
    stepperY.step(160, BACKWARD, STEP);
    CELL = 0;
    horsteps=0;
...
void PenPressing(){
     for (pos = 0; pos <= DOWN; pos+=2) { // goes from 0 degrees to 180 degrees
    // in steps of 1 degree
    penServo.write(pos);              // tell servo to go to position in variable 'pos'
    delay(15);                       // waits 15ms for the servo to reach the position
  }
  for (pos = DOWN; pos >= 0; pos-=2) { // goes from 180 degrees to 0 degrees
    penServo.write(pos);              // tell servo to go to position in variable 'pos'
    delay(15);                       // waits 15ms for the servo to reach the position
  }
 }
